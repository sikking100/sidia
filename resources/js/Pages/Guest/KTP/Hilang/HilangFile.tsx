...
interface HilangFileProps {
  file_family_card: File | null
  file_id_card: File | null
  file_lost_letter: File | null
}

export default function HilangFile({ application }: Props) {
  console.log(application.id)
  const [showAlert, setShowAlert] = React.useState(true)
  const { flash } = usePage().props
  const f = flash as { message: string }

  const { data, setData, processing } = useForm<HilangFileProps>({
    file_family_card: null,
    file_id_card: null,
    file_lost_letter: null,
  })
  ...
  function onSubmit(e: React.ChangeEvent<HTMLFormElement>) {
    ...
    Inertia.post(route('rusak.action', application.id), {
      ...
      'file_lost_letter': data.file_lost_letter,
    ...
    <Guest
      ...
      <Container>
        ...
        <form onSubmit={onSubmit}>
          ...
          />
          <Label forInput={'Surat keterangan hilang dari kepolisian'} className={'w-full pt-6 pb-2'} value={'Surat keterangan hilang dari kepolisian'} />
          <Input
            className={'files'}
            type={'file'}
            handleChange={e => {
              const files = e.target.files
              if (files != null) {
                setData('file_lost_letter', files[0])
              }
            }}
            name={'file_lost_letter'}
          />
